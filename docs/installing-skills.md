...
The easiest and fastest way to install the SharpPragma skills for both Gemini CLI and Claude Code is using `npx`. This works cross-platform (Windows, macOS, Linux) without needing to clone the repository:

```bash
npx @outsourced-br/sharp-pragma-skills
```

This command will automatically download and copy the `sharp-*` folders into your `~/.gemini/skills/` and `~/.claude/skills/` directories.
